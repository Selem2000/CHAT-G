...
exports.getAllUsers = async (req, res) => {
  try {
    let userList = await User.find().populate("FriendList");
...
exports.getUsersByName = async (req, res) => {
  try {
    let usersToFind = await User.find({ firstName: req.params.name });
    usersToFind
      ? res.status(500).send({ msg: "no users" })
      : res.status(200).send({ msg: "get user by name", user: userToFind });
  } catch (error) {
    res.status(400).send({ errors: [{ msg: "faild to get any user" }] });
  }
};
